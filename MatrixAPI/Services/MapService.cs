...
namespace MatrixAPI.Services
{
  public class MapService : IMapService
  {
    ...
    {
      return new MatrixDto(matrix.Id, ToGroupsDto(matrix.Groups), ToControlsDto(matrix.Controls));
    }

    public List<GroupDto> ToGroupsDto(List<Group> groups)
    {
      return groups.Select(g => new GroupDto(g.Id, ToUnitsDto(g.Units), ToControlsDto(g.Controls))).ToList();
    }

    public List<UnitDto> ToUnitsDto(List<Unit> units)
    {
      return units.Select(u => new UnitDto(u.Id, ToControlsDto(u.Controls))).ToList();
    }

    public List<ControlDto> ToControlsDto(List<Control> controls)
    {
      return controls.Select(c => new ControlDto(c.Id, c.Indicator, c.Data)).ToList();
    }
  }
